...
// Cache the resolved lib so we don't re-run dynamic import() resolution
// (including failed-import rejection cost when a lib isn't installed) on
// every image-processing call. Resolved once per process lifetime.
let imageProcessingLibraryPromise;
const getImageProcessingLibrary = async () => {
    if (!imageProcessingLibraryPromise) {
        imageProcessingLibraryPromise = (async () => {
            //@ts-ignore
        ...
        // Don't cache a rejected resolution — allow retry on next call
        // (e.g. if the lib gets installed later without a process restart).
        imageProcessingLibraryPromise.catch(() => {
            imageProcessingLibraryPromise = undefined;
        });
    }
    return imageProcessingLibraryPromise;
};
...
export const extractImageThumb = async (bufferOrFilePath, width = 32) => {
    ...
    const sharpAvailable = 'sharp' in lib && typeof lib.sharp?.default === 'function';
    // sharp can consume a Readable directly, avoiding a full-buffer load into
    // memory. Only fall back to buffering the stream when sharp isn't
    // available (jimp requires a Buffer).
    if (bufferOrFilePath instanceof Readable && !sharpAvailable) {
