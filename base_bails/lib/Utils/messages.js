...
/**
 * EXPERIMENTAL — builds an aiMediaCollectionMessage field object.
 *
 * The WAProto schema for this message (collectionId, expectedMediaCount,
 * hasGlobalCaption) has no known public reference implementation (not in
 * upstream WhiskeySockets/Baileys, not in any other fork as of the time this
 * was written) — the field exists in WAProto because it's already reachable
 * from the server's protocol surface, not because its client-side behavior
 * has been verified against a real WhatsApp client.
 *
 * This is intentionally NOT wired into generateWAMessageContent's dispatch —
 * it's a standalone, opt-in builder so a bad assumption here can't affect
 * the main message-sending path. Attach the result to
 * `message.aiMediaCollectionMessage` yourself before sending, and treat any
 * behavior (including whether WhatsApp's servers accept/relay it at all) as
 * unverified until confirmed against real traffic.
 *
 * @param {{ collectionId: string, expectedMediaCount?: number, hasGlobalCaption?: boolean }} params
 */
export const buildAIMediaCollectionMessage = ({ collectionId, expectedMediaCount, hasGlobalCaption }) => {
    if (!collectionId) {
        throw new Boom('collectionId is required for AIMediaCollectionMessage', { statusCode: 400 });
    }
    return proto.AIMediaCollectionMessage.fromObject({
        collectionId,
        ...(expectedMediaCount !== undefined ? { expectedMediaCount } : {}),
        ...(hasGlobalCaption !== undefined ? { hasGlobalCaption } : {})
    });
};
