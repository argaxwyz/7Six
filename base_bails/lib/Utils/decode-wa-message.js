...
// Cap for the global botMessageSecrets map (separate from the per-chat cap
// below). Currently write-only — no code path reads from this map yet — but
// kept for future-proofing. Bounded with FIFO eviction so it can't grow
// unbounded in long-running processes.
const MAX_BOT_MESSAGE_SECRETS = 500
...
export const setBotMessageSecret = (id, secret, chatJid) => {
	...
	if (!botMessageSecrets.has(id) && botMessageSecrets.size >= MAX_BOT_MESSAGE_SECRETS) {
		const oldestKey = botMessageSecrets.keys().next().value
		botMessageSecrets.delete(oldestKey)
	}
